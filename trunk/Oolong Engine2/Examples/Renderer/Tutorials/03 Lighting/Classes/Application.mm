...
const char* pszAttribs[] = { "myVertex", "myNormal", "myTexCoord" };
...
bool CShell::InitApplication()
{
	...
	{
		/* Gets the Data Path */	
		if(ShaderLoadFromFile("blank", "/LightFragShader.fsh", GL_FRAGMENT_SHADER, 0, &uiFragShader) == 0)
			printf("Loading the fragment shader fails");
		if(ShaderLoadFromFile("blank", "/LightVertShader.vsh", GL_VERTEX_SHADER, 0, &uiVertShader) == 0)
	...
	// We're going to draw a cube, so let's create a vertex buffer first!
	{
		...
		unsigned int uiSize = 24 * (sizeof(GLfloat) * 8);
...
bool CShell::RenderScene()
{
	...
	if( __OPENGLES_VERSION >= 2 ) 
	{
		...
		glVertexAttribPointer(VERTEX_ARRAY, 3, GL_FLOAT, GL_FALSE, 32, 0);	// Stride = 32 bytes

		glEnableVertexAttribArray(NORMAL_ARRAY);
		glVertexAttribPointer(NORMAL_ARRAY, 3, GL_FLOAT, GL_FALSE, 32, (void *)12);	// Stride = 32 bytes
		
		glEnableVertexAttribArray(TEXCOORD_ARRAY);
		glVertexAttribPointer(TEXCOORD_ARRAY, 2, GL_FLOAT, GL_FALSE, 32, (void *)24);
...
#endif
	{
		...
		glVertexPointer(3, GL_FLOAT, 32, 0);	// Stride = 32 bytes
	...
	AppDisplayText->DisplayDefaultTitle("Lighting", "", eDisplayTextLogoIMG);
