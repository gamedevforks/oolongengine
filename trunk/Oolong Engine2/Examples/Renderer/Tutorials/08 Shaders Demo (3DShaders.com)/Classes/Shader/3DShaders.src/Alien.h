...
class TAlien : public TModel {
  public:
    TAlien(unsigned glid, Id id);
    void Draw() const;
    void Update();
    void Reset();
    virtual void Refresh() { dirty = true; Load(-1); }
...
private:
    ...
    mutable TUniform index0; // integer pointing to the first transform matrix
    mutable TUniform index1; // integer pointing to the second transform matrix
    TUniform transforms[NumAngles + 1]; // transform[0] is for the body; the others are for the joints
