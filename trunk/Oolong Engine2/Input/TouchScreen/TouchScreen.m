...
//
// the GSEvent* functions can not be called in a mixed Obj-C/C++ file == *.mm file
// therefore we have those wrappers here ...
//
-  (CGRect) GetLocationInWindow:(GSEvent*)event
{
	return GSEventGetLocationInWindow(event);
}

- (int) GetDeltaX:(GSEvent*)event
{
   return GSEventGetDeltaX(event);
}

- (int) GetDeltaY:(GSEvent*)event
{
   return GSEventGetDeltaY(event);
}
