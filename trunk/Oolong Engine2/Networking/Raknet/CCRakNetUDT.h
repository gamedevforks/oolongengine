...
class CCRakNetUDT
{
	...
	void Init(RakNetTimeUS curTime, uint32_t maxDatagramPayload);
	...
	uint32_t GetRetransmissionBandwidth(RakNetTimeUS curTime, RakNetTimeUS estimatedTimeToNextTick);
	...
	uint32_t GetNewTransmissionBandwidth(RakNetTimeUS curTime, RakNetTimeUS estimatedTimeToNextTick, RakNetTimeUS timeSinceLastContinualSend, uint32_t unacknowledgedBytes);
	...
	bool ShouldSendACKs(RakNetTimeUS curTime, RakNetTimeUS estimatedTimeToNextTick);
	...
	void OnSendBytes(RakNetTimeUS curTime, uint32_t numBytes);
	...
	void OnGotPacket(DatagramSequenceNumberType datagramSequenceNumber, bool isContinuousSend, RakNetTimeUS curTime, uint32_t sizeInBytes, uint32_t *skippedMessageCount);
	...
	void OnAck(RakNetTimeUS curTime, RakNetTimeUS rtt, bool hasBAndAS, BytesPerMicrosecond _B, BytesPerMicrosecond _AS, double totalUserDataBytesAcked );
	
	/// Call when you send an ack, to see if the ack should have the B and AS parameters transmitted
	/// Call before calling OnSendAck()
	void OnSendAckGetBAndAS(RakNetTimeUS curTime, bool *hasBAndAS, BytesPerMicrosecond *_B, BytesPerMicrosecond *_AS);
	...
	void OnSendAck(RakNetTimeUS curTime, uint32_t numBytes);

	/// Call when we send a NACK
	/// Also updates SND, the period between sends, since data is written out
	void OnSendNACK(RakNetTimeUS curTime, uint32_t numBytes);
	...
	BytesPerMicrosecond GetLocalReceiveRate(RakNetTimeUS currentTime) const;
	...
	RakNetTimeUS GetNextAllowedSend(void) const {return nextAllowedSend;}
	...
	RakNetTimeUS continuousBytesReceivedStartTime;
	...
	BytesPerMicrosecond ReceiverCalculateDataArrivalRate(RakNetTimeUS curTime) const;
	...
	void UpdateWindowSizeAndAckOnAckPerSyn(RakNetTimeUS curTime);
	...
	void UpdateNextAllowedSend(RakNetTimeUS curTime, uint32_t numBytes);
