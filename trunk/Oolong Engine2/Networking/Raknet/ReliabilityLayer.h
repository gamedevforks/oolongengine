...
// TODO - Estimated link capacity always too high
...
private:
	...
	BitSize_t WriteToThreadBlockFromInternalPacket( SendToThread::SendToThreadBlock *sttb, const InternalPacket *const internalPacket, RakNetTimeUS curTime );
#endif
	...
	unsigned RemovePacketFromResendListAndDeleteOlderReliableSequenced( const MessageNumberType messageNumber, RakNetTimeUS time, DataStructures::List<PluginInterface2*> &messageHandlerList, SystemAddress systemAddress );

	/// Acknowledge receipt of the packet with the specified messageNumber
	void SendAcknowledgementPacket( const DatagramSequenceNumberType messageNumber, RakNetTimeUS time);
	...
	InternalPacket * BuildPacketFromSplitPacketList( SplitPacketChannel *splitPacketChannel, RakNetTimeUS time );
	...
	void InsertPacketIntoResendList( InternalPacket *internalPacket, RakNetTimeUS time, bool makeCopyOfInternalPacket, bool firstResend, bool modifyUnacknowledgedBytes );
	...
	RakNetTimeUS timeResendQueueNonEmpty;
...
//	RakNetTimeUS lastTimeBetweenPacketsIncrease,lastTimeBetweenPacketsDecrease;
	...
	void PushPacket(RakNetTimeUS time, InternalPacket *internalPacket, bool isReliable);
	...
	void SendACKs(SOCKET s, SystemAddress systemAddress, RakNetTimeUS time, RakNetRandom *rnr, unsigned short remotePortRakNetWasStartedOn_PS3);
	...
	RakNetTimeUS timeOfLastContinualSend;
